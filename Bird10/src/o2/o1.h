...
public:
    ...
    QByteArray generateSignature(const QList<O0RequestParameter> headers, const QUrl &req, const QList<O0RequestParameter> &signingParameters, QNetworkAccessManager::Operation operation);
...
protected Q_SLOTS:
    ...
    virtual void onTokenRequestError(CURLcode error);

    /// Handle token request finished.
    virtual void onTokenRequestFinished(CURLcode code);

    /// Handle token exchange error.
    void onTokenExchangeError(CURLcode code);

    /// Handle token exchange finished.
    void onTokenExchangeFinished(CURLcode code);
