...
QByteArray O0BaseAuth::createQueryParameters(const QList<O0RequestParameter> &parameters) {
    ...
    foreach (O0RequestParameter h, parameters) {
        ...
        //qDebug()<<h.name<<":"<<QUrl::toPercentEncoding(QString::fromUtf8(h.value, h.value.size())).size();
        //qDebug()<<h.name<<":"<<h.value.toPercentEncoding().size();
        ret.append(QUrl::toPercentEncoding(h.name) + "=" + h.value.toPercentEncoding());
    }
    return ret;
}
